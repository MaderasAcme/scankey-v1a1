#!/usr/bin/env python3
"""
Extrae referencias JMA del catálogo C12 - Bloque 02.
Fuente: data/raw/catalogs/c12/catalogo-llaves-c12.pdf (páginas 52-84)
No inventa datos. Marca dudosas con needs_review.
Salida: jma_c12_catalog_part_02.json
"""
import json
import re
...
OUT_PATH = Path(__file__).parent / "jma_c12_catalog_part_02.json"
...
# Rango esperado bloque 02 (1-based)
...
# Páginas bloque 02 (0-based: 51-83 → 1-based: 52-84)
PAGE_RANGE = range(51, 84)


def extract_from_pdf(pdf_path: Path) -> tuple[set[str], dict[str, int]]:
    """Lee el PDF y extrae referencias de las páginas del bloque 02."""
...
def main():
    ...
    print(f"Leyendo PDF: {PDF_PATH} (bloque 02: páginas 52-84)")
    refs, ref_to_page = extract_from_pdf(PDF_PATH)

    # Verificación de contaminación: solo refs con catalog_page en rango 52-84
    out_of_range = [(r, ref_to_page[r]) for r in ref_to_page if ref_to_page[r] < BLOCK_02_PAGE_MIN or ref_to_page[r] > BLOCK_02_PAGE_MAX]
    if out_of_range:
        raise SystemExit(
            f"CONTAMINACIÓN: Se detectaron referencias fuera del rango 52-84:\n"
