#!/usr/bin/env python3
"""
Extrae referencias JMA del catálogo C12 - Bloque 04.
Fuente: data/raw/catalogs/c12/catalogo-llaves-c12.pdf (páginas 118-150)
No inventa datos. Marca dudosas con needs_review.
Salida: jma_c12_catalog_part_04.json
"""
import json
import re
...
OUT_PATH = Path(__file__).parent / "jma_c12_catalog_part_04.json"
...
# Rango esperado bloque 04 (1-based)
...
# Páginas bloque 04 (0-based: 117-149 → 1-based: 118-150)
PAGE_RANGE = range(117, 150)


def extract_from_pdf(pdf_path: Path) -> tuple[set[str], dict[str, int]]:
    """Lee el PDF y extrae referencias de las páginas del bloque 04."""
...
def main():
    ...
    print(f"Leyendo PDF: {PDF_PATH} (bloque 04: páginas 118-150)")
    refs, ref_to_page = extract_from_pdf(PDF_PATH)

    # Verificación de contaminación: solo refs con catalog_page en rango 118-150
    out_of_range = [(r, ref_to_page[r]) for r in ref_to_page if ref_to_page[r] < BLOCK_04_PAGE_MIN or ref_to_page[r] > BLOCK_04_PAGE_MAX]
    if out_of_range:
        raise SystemExit(
            f"CONTAMINACIÓN: Se detectaron referencias fuera del rango 118-150:\n"
