#!/usr/bin/env python3
"""Auditoría del bloque 04 del catálogo JMA C12."""
...
INPUT = Path(__file__).parent / "jma_c12_catalog_part_04.json"
OUT_JSON = Path(__file__).parent / "jma_c12_catalog_part_04_audit.json"
OUT_MD = Path(__file__).parent / "jma_c12_catalog_part_04_audit.md"

# Rango esperado bloque 04
BLOCK_04_PAGE_MIN = 118
BLOCK_04_PAGE_MAX = 150
...
def main():
    ...
    # 0. Verificación de contaminación (páginas fuera de 118-150)
    catalog_pages = [r.get("catalog_page") for r in refs if r.get("catalog_page") is not None]
    out_of_range = [p for p in catalog_pages if p < BLOCK_04_PAGE_MIN or p > BLOCK_04_PAGE_MAX]
    contamination = len(out_of_range) > 0
    if contamination:
        unique_bad = sorted(set(out_of_range))
        raise SystemExit(
            f"CONTAMINACIÓN: Páginas fuera del rango 118-150: {unique_bad[:30]}{'...' if len(unique_bad) > 30 else ''}\n"
    ...
    audit = {
        ...
        "contamination_out_of_range": contamination,
    ...
    # Generar audit MD
    lines = [
        "# Auditoría: jma_c12_catalog_part_04.json",
        "",
        "## 0. Resumen",
        f"- **total_refs**: **{total}**",
        ...
        f"- **Contaminación fuera de rango 118-150**: **{'sí' if contamination else 'no'}**",
        "",
        "## 1. id_model_ref únicos",
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Generado por audit_block_04.py*")
