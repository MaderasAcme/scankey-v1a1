...
import { AUTO_CAPTURE_ENABLED_KEY } from '../utils/autoCapture';
...
export const ProfileModal = memo(({ isOpen, onClose, onLogout, onResetData, onFlushQueue, onViewLast }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setAutoCaptureEnabled(Boolean(s[AUTO_CAPTURE_ENABLED_KEY]));
    }
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-2xl z-[100] flex items-end animate-in fade-in duration-300">
      ...
      >
        ...
        <div className="bg-zinc-900/20 border border-zinc-800 rounded-3xl p-6 mb-4 space-y-4">
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm text-zinc-400">Auto-captura</span>
            <input
              type="checkbox"
              checked={autoCaptureEnabled}
              onChange={(e) => {
                const v = e.target.checked;
                setAutoCaptureEnabled(v);
                saveSettings({ [AUTO_CAPTURE_ENABLED_KEY]: v });
              }}
              className="rounded"
            />
          </div>
